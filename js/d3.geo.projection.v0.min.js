function() {
    ...
    return arguments.length < 8 && (h = 0), u.invert = function(u, s) {
      ...
      do {
        var d = l * l,
          ...
          S = Math.asin(b / m) / fa,
          Q = v * S;
        if (xB2 = b * b, dxBdÏ† = (2 * n + d * (4 * a + 6 * d * r)) * l, dyBdÏ† = e + d * (3 * o + 5 * d * i), dpdÏ† = 2 * (b * dxBdÏ† + p * (dyBdÏ† - 1)), dqdÏ† = 2 * (dyBdÏ† - 1), dmdÏ† = (dpdÏ†*q - w * dqdÏ†) / (q * q), cosÎ± = Math.cos(Q), sinÎ± = Math.sin(Q), mcosÎ± = m * cosÎ±, msinÎ± = m * sinÎ±, dÎ±dÏ† = v / fa * (1 / M(1 - xB2 / y)) * (dxBdÏ†*m - b * dmdÏ†) / y, fx = msinÎ± - u, fy = l * (1 + d * h) + m - mcosÎ± - s, Î´xÎ´Ï† = dmdÏ†*sinÎ± + mcosÎ±*dÎ±dÏ†, Î´xÎ´Î» = mcosÎ±*S, Î´yÎ´Ï† = 1 + dmdÏ† - (dmdÏ†*cosÎ± - msinÎ±*dÎ±dÏ†), Î´yÎ´Î» = msinÎ±*S, denominator = Î´xÎ´Ï†*Î´yÎ´Î» - Î´yÎ´Ï†*Î´xÎ´Î», !denominator) break;
        v -= c = (fy * Î´xÎ´Ï† - fx * Î´yÎ´Ï†) / denominator, l -= f = (fx * Î´yÎ´Î» - fy * Î´xÎ´Î») / denominator
  ...
  function rn(t, n) {
    ...
    e = i * u[c] * t;
    do o = s[c] * Math.sin(r = e) / u[c], e = .5 * (h(o) + e);
    while (--c);
    ...
    return u.stream = function(t) {
      ...
      return arguments.length ? (h = t.map(function(t) {
        return t.map(function(t) {
          return [[t[0][0] * fa / 180, t[0][1] * fa / 180], [t[1][0] * fa / 180, t[1][1] * fa / 180], [t[2][0] * fa / 180, t[2][1] * fa / 180]]
        })
      }), a(), u) : h.map(function(t) {
        return t.map(function(t) {
          return [[180 * t[0][0] / fa, 180 * t[0][1] / fa], [180 * t[1][0] / fa, 180 * t[1][1] / fa], [180 * t[2][0] / fa, 180 * t[2][1] / fa]]
        })
      })
    }, u
  ...
  (d3.geo.mollweide = function() {
    ...
    var a, r, e = 2.00276,
      ...
      i = 0 > n ? -fa / 4 : fa / 4,
      h = 25;
    do r = o - Math.SQRT2 * Math.sin(i), i -= a = (Math.sin(2 * i) + 2 * i - fa * Math.sin(r)) / (2 * Math.cos(2 * i) + 2 + fa * Math.cos(r) * Math.SQRT2 * Math.cos(i));
    while (Math.abs(a) > sa && --h > 0);
  ...
  Ga.forEach(function(t) {
    ...
    do {
      ...
      if (v >= 0 || 1 === o) {
        r = (n >= 0 ? 5 : -5) * (v + e);
        var l, g = 50;
        do e = Math.min(18, Math.abs(r) / 5), o = Math.floor(e), v = e - o, i = Ga[o][1], h = Ga[o + 1][1], u = Ga[Math.min(19, o + 2)][1], r -= (l = (n >= 0 ? va : -va) * (h + v * (u - i) / 2 + v * v * (u - 2 * h + i) / 2) - n) * da;
        while (Math.abs(l) > ca && --g > 0);
        break
      }
    } while (--o >= 0);
    var d = Ga[o][0],
